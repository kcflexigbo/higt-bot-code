"""Phase 6.4 — train HiGTBotSparsePool (SAGPool-based hierarchical pooling).

Mirrors scripts/train_phase6.py but swaps the DiffPool model for the sparse
SAGPool variant defined in src/models/sparse_pool.py. No aux losses (link /
entropy) — sparse pooling is trained end-to-end through the classification
loss only.

Per-node prediction is preserved: every input node gets a logit. The pooling
only changes the graph-level *summary* fed alongside the per-node embedding.

Usage
-----
uv run python scripts/train_phase6_sparse.py --emb-dir data/flow_embeddings
uv run python scripts/train_phase6_sparse.py --emb-dir data/flow_embeddings_ssl_ft \
    --pool-ratio 0.5 --results-suffix _ssl_ft
"""
...
RESULTS = Path("data/inspection_logs/phase6_sparse.json")
...
def main() -> None:
    ...
    ap.add_argument("--pool-ratio", type=float, default=0.5,
                    help="fraction of nodes kept per pooling layer")
    ...
    print(f"device: {device}  amp={use_amp}  emb_dir={args.emb_dir}", flush=True)
    ...
    print(f"HiGTBotSparsePool params: {n_params:,}  pool_ratio={args.pool_ratio}",
          flush=True)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"[sparse] epoch {epoch:>3d}  ce {ep_loss/max(n_b,1):.4f}  "
              f"val_F1 {val_f1:.4f}  best {best_f1:.4f}  lr {lr_now:.1e}  "
              f"{'*' if imp else ' '}  {dt:.1f}s", flush=True)
        if bad >= args.patience:
            print(f"[sparse] early stop at epoch {epoch}")
            break
    ...
    print(); print(val_res.pretty("val (hgt-sparse)"))
    print(); print(test_res.pretty("test (hgt-sparse)"))

    CKPT_DIR.mkdir(parents=True, exist_ok=True)
    ckpt = CKPT_DIR / f"hgt_sparse{args.results_suffix}.pt"
    torch.save({"state_dict": model.state_dict(), "cfg": cfg,
                 "args": vars(args), "edge_scaler": (e_mean, e_std),
                 "node_scaler": (n_mean, n_std)}, ckpt)
    print(f"saved {ckpt}")

    results_path = RESULTS.with_name(f"phase6_sparse{args.results_suffix}.json")
