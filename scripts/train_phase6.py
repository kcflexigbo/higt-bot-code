"""Phase 6: train HiGTBotDiffPool on top of cached encoder embeddings.

Per the implementation plan:
  - Load cached [N, d_model] embeddings from data/flow_embeddings/
  - Concat with raw scaled node features
  - 1-level DiffPool with link + entropy aux losses
  - Per-node CE / focal loss as primary objective
  - Mixed precision, gradient accumulation

Gate (per plan): match or beat flat GIN on test F1.
"""
...
from src.training.tam_loss import TopologyAwareFocalLoss
from src.training.graphsha import minority_manifold_mixup
from src.utils.seeding import pick_device, set_seed

CKPT_DIR = Path("experiments/phase6")
RESULTS = Path("data/inspection_logs/phase6_diffpool.json")
DEFAULT_CONFIG = Path("configs/phase5.yaml")  # reuse encoder dims; phase6.yaml later
...
def main() -> None:
    ...
    ap.add_argument("--w-link", type=float, default=0.1)
    ap.add_argument("--w-ent", type=float, default=0.1)
    ap.add_argument("--no-amp", action="store_true")
    ap.add_argument("--tam", action="store_true",
                    help="use Topology-Aware Margin focal loss (Song+ ICML22)")
    ap.add_argument("--tam-alpha", type=float, default=1.5,
                    help="TAM margin strength (ACM term)")
    ap.add_argument("--tam-ccm-beta", type=float, default=0.5,
                    help="TAM CCM (per-class) scaling")
    ap.add_argument("--tam-balanced-prior", action="store_true",
                    help="Use pi_global=[0.5,0.5] instead of empirical class freqs "
                         "(prevents TAM from amplifying the dataset's own imbalance)")
    ap.add_argument("--graphsha", action="store_true",
                    help="enable GraphSHA-lite intra-graph minority manifold mixup")
    ap.add_argument("--graphsha-beta", type=float, default=1.0,
                    help="Beta(α,α) shape for mixup λ; 1.0=uniform, >1 concentrates "
                         "near 0.5 (more mixing)")
    ap.add_argument("--graphsha-prob", type=float, default=0.5,
                    help="per-minority-node Bernoulli probability of applying mixup")
    ap.add_argument("--results-suffix", type=str, default="")
    ap.add_argument("--emb-dir", type=Path, default=Path("data/flow_embeddings"),
                    help="directory of cached encoder embeddings to consume")
    args = ap.parse_args()
    ...
    print(f"emb_dir={args.emb_dir}", flush=True)
    ...
    # Probe d_model from one embedding
    probe = tr_ds[0]
    d_model = int(probe.node_emb.size(1))
    print(f"d_model={d_model}  raw_feat_dim={raw_feat_dim}  edge_dim={edge_dim}", flush=True)

    model = HiGTBotDiffPool(
        d_model=d_model, raw_feat_dim=raw_feat_dim, edge_dim=edge_dim,
        hidden=args.hidden, max_nodes=args.max_nodes, pool_ratio=args.pool_ratio,
        num_classes=2, dropout=args.dropout,
    ).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"HiGTBotDiffPool params: {n_params:,}  K1={model.K1}", flush=True)
    ...
    if args.tam:
        # Global class prior from training labels (probe via class_weight inverse).
        # class_weight is mean-normalized inverse-frequency, so the implied
        # frequencies are inv-weights renormalized to sum to 1.
        if args.tam_balanced_prior:
            pi_global = torch.tensor([0.5, 0.5], dtype=torch.float32)
        else:
            inv = 1.0 / torch.tensor(class_weight, dtype=torch.float32)
            pi_global = inv / inv.sum()
        loss_fn = TopologyAwareFocalLoss(
            gamma=2.0, alpha_focal=alpha, pi_global=pi_global,
            tam_alpha=args.tam_alpha, use_ccm=True, ccm_beta=args.tam_ccm_beta,
        )
        print(f"TAM focal loss  gamma=2.0  alpha={alpha.tolist()}  "
              f"pi_global={pi_global.tolist()}  tam_alpha={args.tam_alpha}  "
              f"ccm_beta={args.tam_ccm_beta}", flush=True)
    else:
    ...
    for epoch in range(1, args.epochs + 1):
        model.train()
        ep_loss, ep_link, ep_ent = 0.0, 0.0, 0.0
        n_b = 0
        ...
        for step, batch in enumerate(train_loader, start=1):
            ...
            if args.graphsha:
                batch.node_emb, batch.x = minority_manifold_mixup(
                    batch.node_emb, batch.x, batch.y, batch.batch,
                    pos_class=1, beta_alpha=args.graphsha_beta,
                    mix_prob=args.graphsha_prob,
                )
            with _amp_ctx(device, use_amp):
                out = model(batch)
                if args.tam:
                    ce = loss_fn(out["logits"], batch.y, batch.edge_index)
                else:
                    ce = loss_fn(out["logits"], batch.y)
                loss = (ce + args.w_link * out["link_loss"] + args.w_ent * out["ent_loss"]) / args.grad_accum
            if use_amp:
                scaler.scale(loss).backward()
            else:
                loss.backward()
            ep_loss += float(ce.item())
            ep_link += float(out["link_loss"].item())
            ep_ent += float(out["ent_loss"].item())
            n_b += 1
        ...
        print(f"[hgt] epoch {epoch:>3d}  ce {ep_loss/max(n_b,1):.4f}  link {ep_link/max(n_b,1):.4f}  "
              f"ent {ep_ent/max(n_b,1):.4f}  val_F1 {val_f1:.4f}  best {best_f1:.4f}  "
              f"lr {lr_now:.1e}  {'*' if imp else ' '}  {dt:.1f}s", flush=True)
        if bad >= args.patience:
            print(f"[hgt] early stop at epoch {epoch}")
            break
    ...
    print(); print(val_res.pretty("val (hgt-diffpool)"))
    print(); print(test_res.pretty("test (hgt-diffpool)"))

    CKPT_DIR.mkdir(parents=True, exist_ok=True)
    ckpt = CKPT_DIR / f"hgt_diffpool{args.results_suffix}.pt"
    torch.save({"state_dict": model.state_dict(), "cfg": cfg,
                 "args": vars(args), "edge_scaler": (e_mean, e_std),
                 "node_scaler": (n_mean, n_std)}, ckpt)
    print(f"saved {ckpt}")

    results_path = RESULTS.with_name(f"phase6_diffpool{args.results_suffix}.json")
