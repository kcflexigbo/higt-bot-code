...
def _proto_onehot(protocol: pd.Series) -> np.ndarray:
    ...
    for i, p in enumerate(protocol.values):
        out[i, _PROTO_IDX.get(p, _PROTO_IDX["other"])] = 1.0
    return out
...
def flow_features_for_node(
    ...
    *,
    ...
    # window_start may be tz-aware; convert both sides to numeric UTC seconds.
    ws_utc = window_start.tz_convert("UTC") if window_start.tzinfo is not None else window_start.tz_localize("UTC")
